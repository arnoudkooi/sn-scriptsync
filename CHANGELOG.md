...
**Find the Debug edition build for the browser debugger (beta):** The Welcome / What's New tab and the `sn-scriptsync.browserDebugger.enabled` setting now link straight to the [SN Utils Debug edition browser build](https://chromewebstore.google.com/detail/sn-utils-debug/imjkemgdgfakdbobaoagilnoanibajeb) — the build that ships the CDP debugger adapter. Without it the browser-debugger commands return `E_CDP_UNAVAILABLE` even with Pro enabled and the setting on, so the link makes the missing piece obvious.

**Capabilities highlight in the browser sync log:** When the SN Utils helper tab connects, ScriptSync now adds a highlighted row to the sync log summarising the newer Agent API capabilities (build/edit artifacts, live-form control, code search, and the browser-debugger beta) with a link to the Debug edition build, and the connect banner was refreshed from the old v4.3.0 copy to reflect them.
