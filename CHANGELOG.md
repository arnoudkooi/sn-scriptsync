...
- **Sessions come back on their own after a restart.** The bridge names itself to the SN Utils helper tab, which refreshes session tokens automatically when it reconnects to a bridge you already sent a `/token` to. Restarting an agent, reloading the helper tab or waking the laptop no longer needs a new `/token`. Requires SN Utils 10.2.3.5 and an open tab of the instance. Both hosts refuse WebSocket connections from web pages, including sandboxed pages. The shared bridge identity stays private and stable when hosts start simultaneously.
- **Two helper tabs no longer replace each other every second.** A helper tab that was just opened still takes over, and the replaced tab is told why. Its automatic reconnects are turned away while another helper tab is live, so tabs in separate Chrome profiles stop fighting over the connection. Requires SN Utils 10.2.3.5 for the standby behaviour.
- **`snu doctor` reports the bridge identity.** A missing, invalid or non-private identity file is explained with the step to fix it, since automatic session refresh stays off without one. The id itself never appears in the report.
