...
async function saveFieldAsFile(postedJson, retry = 0) {
	...
	// If the record already has an entry in _map.json, use the given filename.
	cleanName = Object.keys(nameToSysId).find(fileName => nameToSysId[fileName] === postedJson.sys_id) ?? cleanName

	// Autogenerated file name or existing file name is always safe.
	let nameIsFilesystemSafe = true

	if(settings.get("promptForFileNameOnColision")) {
		// Prompt the user for a new file name, as long as the given file name is already in use by another record,
		// or the file name is invalid. (Contains illegal chars.)
		while(!nameIsFilesystemSafe || nameToSysId[cleanName] && nameToSysId[cleanName] != postedJson.sys_id) {
			const invalidFileNamePrompt = "The name you entered contains illegal characters. Try again.";
			const duplicateFileNamePrompt = `A record named "${cleanName}" already exists. Please provide a unique name.`
			const fileNameInputBoxOptions = {
				prompt: nameIsFilesystemSafe ? duplicateFileNamePrompt : invalidFileNamePrompt,
				value: cleanName,
				placeHolder: "Name goes here"
			}

			cleanName = await window.showInputBox(fileNameInputBoxOptions);
			nameIsFilesystemSafe = eu.isFilesystemSafe(cleanName)
		}
	} else {
		// If mapping already exist add first and last 2 chars of the syid to the filename
		if(nameToSysId[cleanName] && nameToSysId[cleanName] != postedJson.sys_id) {
			cleanName += ("-" + postedJson.sys_id.slice(0,2) + postedJson.sys_id.slice(-2)).toUpperCase();
		}
	}
